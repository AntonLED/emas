...
def asker_agent2(config_path, **kwargs):
    try:
    ...
    return AskerAgent2(name, database_params, power_params, **kwargs)


class AskerAgent2(BaseAsker):
    ...
    def get_default_config(self):
        default_config = {
            ...
            "power_params": {"power": 1.0, "price": 11.0},
        }
    ...
    def switch(self, state: bool):
        ...
        query = f"UPDATE tab2 SET val=%s WHERE no = {Database_commands.relay3_state}"
        vals = [state]
        try:
    ...
    def update_ui(self, price: float):
        if not self.database:
            return
        cursor = self.database.cursor()
        query = f"UPDATE tab2 SET val=%s WHERE no = {Database_commands.asker3_price}"
        vals = [price]
        try:
            cursor.execute(query, vals)
            self.database.commit()
...
def main():
    ...
    utils.vip_main(asker_agent2, version=__version__)
